...
```{r Compare column names each of the files, While the names don't have to be in the same order, they DO need to match perfectly before we can use a command to join them into one file}
...
```{r Stack individual quarter's data frames into one big data frame}
...
```{r   SNotice that the days of the week are out of order. Let's fix that.}
all_trips_v2$day_of_week <- ordered(all_trips_v2$day_of_week, levels=c("星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"))
```

```{r   Now, let's run the average ride time by each day for members vs casual users}
aggregate(all_trips_v2$ride_length ~ all_trips_v2$member_casual + all_trips_v2$day_of_week, FUN = mean)
```
